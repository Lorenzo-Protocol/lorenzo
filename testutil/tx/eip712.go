...
func PrepareEIP712CosmosTx(
	...
	accNumber := appLorenzo.AccountKeeper.GetAccount(ctx, from).GetAccountNumber()

	nonce, err := appLorenzo.AccountKeeper.GetSequence(ctx, from)
	if err != nil {
		return nil, err
	}
...
func createTypedData(args typedDataArgs, useLegacy bool) (apitypes.TypedData, error) {
	if useLegacy {
		...
		lorenzoCodec := codec.NewProtoCodec(registry)
