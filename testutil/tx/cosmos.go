...
type CosmosTxArgs struct {
	...
	// ChainID is the chain's id on cosmos format, e.g. 'lorenzo_8329-1'
	ChainID string
...
func signCosmosTx(
	...
	seq, err := appLorenzo.AccountKeeper.GetSequence(ctx, addr)
	if err != nil {
		return nil, err
	}
	...
	accNumber := appLorenzo.AccountKeeper.GetAccount(ctx, addr).GetAccountNumber()
