...
func DefaultConfig() Config {
	...
	chainID := fmt.Sprintf("lorenzo_%d-1", tmrand.Int63n(9999999999999)+1)
	return Config{
		Codec:             encCfg.Codec,
		...
		AccountTokens:     sdk.TokensFromConsensusPower(1000000000000000000, etherminttypes.PowerReduction),
		StakingTokens:     sdk.TokensFromConsensusPower(500000000000000000, etherminttypes.PowerReduction),
		BondedTokens:      sdk.TokensFromConsensusPower(100000000000000000, etherminttypes.PowerReduction),
...
func NewAppConstructor(encodingCfg appparams.EncodingConfig, chainID string) AppConstructor {
...
func New(l Logger, baseDir string, cfg Config) (*Network, error) { //nolint:gocyclo
	// only one caller/test can create and use a network at a time
	l.Log("acquiring test network lock")
	lock.Lock()

	if !etherminttypes.IsValidChainID(cfg.ChainID) {
	...
	for i := 0; i < cfg.NumValidators; i++ {
		...
		nodeDir := filepath.Join(network.BaseDir, nodeDirName, "lorenzod")
		clientDir := filepath.Join(network.BaseDir, nodeDirName, "lorenzodcli")
		...
		genAccounts = append(genAccounts, &etherminttypes.EthAccount{
