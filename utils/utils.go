...
const (
	// MainnetChainID defines the Lorenzo EIP155 chain ID for mainnet
	MainnetChainID = "lorenzo_8329-1"
	// TestnetChainID defines the Lorenzo EIP155 chain ID for testnet
	TestnetChainID = "lorenzo_83291-1"
	// BaseDenom defines the Lorenzo mainnet denomination
	BaseDenom = "alrz"
)

// IsMainnet returns true if the chain-id has the Lorenzo mainnet EIP155 chain prefix.
func IsMainnet(chainID string) bool {
	return strings.HasPrefix(chainID, MainnetChainID)
}

// IsTestnet returns true if the chain-id has the Lorenzo testnet EIP155 chain prefix.
...
// GetLorenzoAddressFromBech32 returns the sdk.Account address of given address,
// while also changing bech32 human readable prefix (HRP) to the value set on
// the global sdk.Config (eg: `Lorenzo`).
// The function fails if the provided bech32 address is invalid.
func GetLorenzoAddressFromBech32(address string) (sdk.AccAddress, error) {
